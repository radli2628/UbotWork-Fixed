    ...
    if (update.callback_query) {
      ...
      if (isSuperuser) {
        ...
        if (data.startsWith("owner_reject:")) {
          ...
          await db.update(paymentRequestsTable).set({ status: "rejected", rejectionReason: "Rejected by admin.", updatedAt: new Date() }).where(eq(paymentRequestsTable.id, reqId));
          await sendMessage(
            bot.token,
            req.chatId,
            `${settings.errorEmoji} <b>Payment Not Confirmed</b>\n\nYour request #${reqId} was not approved.\n\n<b>Reason:</b> Rejected by admin.\n\nPlease try again or contact support.`,
            { reply_markup: { inline_keyboard: [[{ text: `${settings.planEmoji} View plans`, callback_data: "choose_plan" }]] } },
          );
          await editMessage(bot.token, chatId, messageId, `❌ Request #${reqId} rejected.`, {
            reply_markup: { inline_keyboard: [[{ text: "← Pending", callback_data: "owner_pending" }]] },
          });
          return;
        }
